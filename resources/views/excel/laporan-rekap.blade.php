...
<head>
    <meta charset="UTF-8">
    <title>Rekapitulasi Laporan Desa</title>
    <style>
        body {
...
<body>
    <table style="width: 100%;">
        <tr>
            <td colspan="7" class="header-title">SISTEM INFORMASI PELAPORAN DESA BINAAN IMIGRASI (SIMPEL DBI)</td>
        </tr>
        <tr>
            <td colspan="7" class="header-subtitle">REKAPITULASI DAFTAR LAPORAN DESA BINAAN</td>
        </tr>
        <tr>
            <td colspan="7" class="meta-info">
                Wilayah Desa: <b>{{ $desaNama }}</b> | Tanggal Ekspor: <b>{{ date('d F Y, H:i') }} WIB</b> | Total Laporan: <b>{{ count($laporanList) }} Data</b>
            </td>
        </tr>
        ...
        <tbody>
            @forelse($laporanList as $idx => $item)
                ...
                <tr class="{{ $idx % 2 === 1 ? 'bg-even' : '' }}">
                    ...
                    <td class="text-center">{{ $item->tanggal_kejadian ? $item->tanggal_kejadian->format('d/m/Y H:i') : '-' }}</td>
