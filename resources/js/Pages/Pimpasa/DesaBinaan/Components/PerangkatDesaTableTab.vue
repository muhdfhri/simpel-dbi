...
// --- MODAL DIALOG CRUD USER PERANGKAT DESA PIMPASA ---
...
const submitForm = () => {
    ...
    if (editingDesaUser.value) {
        router.put(`/pimpasa/desa-binaan/perangkat-desa/${editingDesaUser.value.id}`, payload, {
            onSuccess: () => {
        ...
        router.post('/pimpasa/desa-binaan/perangkat-desa', payload, {
            onSuccess: () => {
                isDialogOpen.value = false;
            },
            onError: (errs) => {
...
const executeDelete = () => {
    ...
    router.delete(`/pimpasa/desa-binaan/perangkat-desa/${desaUserToDelete.value.id}`, {
        onSuccess: () => {
...
<template>
    <div class="space-y-4 font-sans">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div class="relative w-full sm:w-72">
                ...
                <Input
                    type="text"
                    v-model="searchInput"
                    placeholder="Cari user perangkat desa binaan..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-xs text-left text-slate-600 font-sans border-collapse">
                ...
                <tbody class="divide-y divide-slate-200/70 bg-white">
                    <tr v-if="processedDesaUserList.length === 0">
                        <td colspan="7" class="py-14 text-center text-slate-400">
                            <UserCheck :size="36" class="mx-auto mb-2 opacity-40 text-slate-400" />
                            <p class="font-bold text-slate-700 text-xs">Belum ada akun user perangkat desa binaan</p>
                            <p class="text-[11px] text-slate-400 mt-0.5">Klik tombol "Tambah Perangkat Desa" untuk mendaftarkan akun baru.</p>
                        </td>
                    </tr>
        ...
        <!-- Pagination Controls -->
        <div v-if="processedDesaUserList.length > 0" class="p-4 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
            <div>
            ...
            <DialogContent class="sm:max-w-[520px] bg-white rounded-lg p-6 sm:p-7 shadow-2xl space-y-0 font-sans border border-slate-100">
                ...
                <DialogHeader class="-mx-6 sm:-mx-7 px-6 sm:px-7 pb-3 border-b border-slate-100 space-y-1 text-left">
                    ...
                    <DialogDescription class="text-xs text-slate-500 leading-relaxed font-normal pt-0.5">
                        Isi data akun login Perangkat Desa & penempatan desa binaannya di wilayah UPT Anda.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form @submit.prevent="submitForm" class="space-y-4 pt-3 text-xs">
                    ...
                    <div>
                        <Label class="font-semibold text-slate-800 text-xs tracking-tight block mb-1.5">Desa Binaan (Wilayah UPT Anda)</Label>
                        <Combobox
