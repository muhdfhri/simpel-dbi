...
const kategoriComboboxOptions = computed(() => [
    {
        value: 'all',
        label: `Semua Kategori (${props.statusCounts?.all ?? props.stats.total})`,
    },
...
<template>
    <AppLayout title="Daftar Laporan Desa">
        <div class="space-y-6 font-sans">
            
            <!-- Quick Metric KPI Cards (Presisi Matching Master Data Admin Kanwil) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card @click="filterByStatus('all')" class="border-slate-200/80 shadow-2xs rounded-lg bg-white relative overflow-hidden group cursor-pointer hover:border-slate-400 transition-all">
                    ...
                    <CardContent class="p-4 sm:p-5 flex flex-col justify-between h-full min-h-[92px] relative z-10">
                        <div class="space-y-0.5 pr-12">
                            <span class="text-xs font-semibold text-slate-500 block leading-tight">Total Pengajuan Tiket</span>
                            <div class="text-3xl font-bold text-slate-900 font-sans tabular-nums tracking-tight leading-none pt-1">
                                {{ stats.total }}
                            </div>
                        </div>
                        <div class="pt-2">
                            <p class="text-[11px] text-slate-500 font-medium">Keseluruhan laporan desa</p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card @click="filterByStatus('diajukan')" class="border-slate-200/80 shadow-2xs rounded-lg bg-white relative overflow-hidden group cursor-pointer hover:border-amber-400 transition-all">
                    ...
                    <CardContent class="p-4 sm:p-5 flex flex-col justify-between h-full min-h-[92px] relative z-10">
                        ...
                        <div class="pt-2">
                            <p class="text-[11px] text-slate-500 font-medium">Proses peninjauan PIMPASA</p>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent class="p-4 sm:p-5 flex flex-col justify-between h-full min-h-[92px] relative z-10">
                        <div class="space-y-0.5 pr-12">
                            ...
                            <div class="text-3xl font-bold text-amber-900 font-sans tabular-nums tracking-tight leading-none pt-1">
                                {{ stats.minta_perbaikan }}
                            </div>
                        </div>
                        <div class="pt-2">
                            <p class="text-[11px] text-amber-800 font-semibold">Perlu tindakan revisi segera</p>
                        </div>
                    </CardContent>
                </Card>

                <!-- Card 4: Selesai Ditindaklanjuti -->
            ...
            <!-- Main Card Section: Data Table (DESIGN.md Primary Navy Button #033566) -->
            <Card class="border-slate-200/80 shadow-2xs rounded-lg bg-white overflow-hidden">
                ...
                <div class="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h3 class="text-base font-bold text-slate-900 tracking-tight">
                            Daftar Laporan Kejadian Desa Binaan
                        </h3>
                        <p class="text-xs text-slate-500 mt-0.5">
                            Kelola dan pantau seluruh pengajuan tiket pelaporan desa binaan Anda.
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                class="h-9 px-3.5 border-emerald-200 bg-emerald-50/70 hover:bg-emerald-100/90 text-emerald-800 font-semibold rounded-md text-xs flex items-center justify-center gap-1.5 shadow-2xs transition-all cursor-pointer flex-1 sm:flex-none"
                            >
                            ...
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                class="h-9 px-3.5 border-rose-200 bg-rose-50/70 hover:bg-rose-100/90 text-rose-800 font-semibold rounded-md text-xs flex items-center justify-center gap-1.5 shadow-2xs transition-all cursor-pointer flex-1 sm:flex-none"
                            >
                        ...
                        <Link href="/desa/laporan/create" class="w-full sm:w-auto">
                            <Button class="w-full sm:w-auto h-9 bg-primary hover:bg-[#04407D] text-primary-foreground font-semibold rounded-md text-xs px-4 flex items-center justify-center gap-1.5 shadow-xs cursor-pointer">
                                <Plus :size="16" />
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr
                                ...
                                :class="[
                                    ...
                                    row.status === 'minta_perbaikan'
                                        ? 'bg-amber-50/50 hover:bg-amber-100/60 border-l-4 border-l-amber-500'
                                        : 'hover:bg-slate-50/70'
                                ]"
                            >
                                <!-- Kolom No (Real Data ID font-sans tabular-nums text-slate-900 font-bold - Inter Font) -->
                                ...
                                <td class="py-4 px-5 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1.5">
                                        <Link v-if="['diajukan', 'minta_perbaikan'].includes(row.status)" :href="`/desa/laporan/${row.id}/edit`">
                                            <Button
                                                :class="[
                                                    'h-8 px-2.5 text-xs font-semibold rounded-md flex items-center gap-1 cursor-pointer transition-all shadow-2xs',
                                                    row.status === 'minta_perbaikan'
                                            ...
                                            >
                                                <FileEdit :size="13" />
                                                <span>{{ row.status === 'minta_perbaikan' ? 'Perbaiki Data' : 'Edit' }}</span>
                                            </Button>
                                        </Link>

                                        <Link :href="`/desa/laporan/${row.id}`">
                                            <Button variant="outline" class="h-8 px-2.5 text-xs font-semibold rounded-md border-slate-300 text-slate-700 hover:bg-slate-100">
                                                <Eye :size="13" class="mr-1" />
                                                Detail Tiket
                                            </Button>
                                        </Link>
                                    </div>
                ...
                <div v-if="laporan.total > 0" class="p-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
                    <div>
                        ...
                        dari <span class="font-bold text-slate-900">{{ laporan.total }}</span> Data Laporan Desa
                    </div>
